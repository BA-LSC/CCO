...
async function ensureOrgUpdateSettingsColumns(): Promise<void> {
...
export async function applyCloudflareReleaseUpdate(): Promise<{
  ...
  const resources = {
    ...
    r2BucketName: org.cloudflareR2BucketName ?? undefined,
    kvPresenceNamespaceId: org.cloudflareKvPresenceNamespaceId ?? undefined,
    kvDeployNamespaceId: org.cloudflareKvDeployNamespaceId ?? undefined,
    pushQueueId: org.cloudflarePushQueueId ?? undefined,
  ...
  try {
    ...
    const assetsManifest = await fetchWebReleaseManifest(
      `${releasesBase}/cco-web-manifest.json`,
    );
    ...
    await db
      .update(organizations)
      .set({
        installedReleaseVersion: releaseIndex.version,
        lastUpdateCheckAt: new Date(),
      })
      .where(eq(organizations.id, org.id));
    invalidateOrgContextCache();

    return { appliedVersion: releaseIndex.version, deployedWorkers };
  } finally {
